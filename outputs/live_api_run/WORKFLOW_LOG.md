...
- Mode: api
- Started: 2026-06-19T09:16:21Z
- Finished: 2026-06-19T09:17:03Z
...
optionally call an OpenAI-compatible LLM API when an API key is available, and
fall back to deterministic mock output when no key is available.

The app should record prompt, model name, settings, generated responses, and
produce a simple comparison report.

The generated project must include app.py, requirements.txt, README.md, and
unittest tests that do not require network access or API keys.

## Planner Output

```json
{
  "title": "LLM Prompt Comparison App",
  "objective": "Build a Streamlit app that allows users to enter a task, compare two prompt variants, optionally call an OpenAI-compatible LLM API when an API key is available, and fall back to deterministic mock output when no key is available. The app should record prompt, model name, settings, generated responses, and produce a simple comparison report. The generated project must include app.py, requirements.txt, README.md, and unittest tests that do not require network access or API keys.",
  "constraints": [
    "No credentials, private paths, SSH, or remote GPU requirements.",
    "Must include app.py, requirements.txt, README.md, and unittest tests.",
    "Tests must not require network access or API keys.",
    "Must fall back to deterministic mock output when no API key is available.",
    "Must record prompt, model name, settings, and generated responses.",
    "Must produce a simple comparison report."
  ],
  "target_stack": [
    "Python 3.9+",
    "Streamlit",
  ...
  "success_criteria": [
    ...
    "User can input a task and two prompt variants.",
    "When API key is set, app calls OpenAI-compatible API and displays responses.",
    "When no API key is set, app uses deterministic mock output.",
    "App records and displays comparison report (prompts, model, settings, responses).",
    "All tests pass without network access or API keys.",
    "Project includes app.py, requirements.txt, README.md, and test files."
  ]
}
```
...
```json
{
  "summary": "Build a Streamlit app for comparing two LLM prompt variants with optional API integration and deterministic fallback, including a comparison report and unit tests.",
  "modules": [
    "app.py",
    "test_app.py",
    "requirements.txt",
  ...
  "data_flow": [
    "User inputs task description and two prompt variants via Streamlit UI",
    "App checks for OPENAI_API_KEY environment variable",
    "If key exists: call OpenAI-compatible API with each prompt variant",
    "If no key: use deterministic mock output generator",
    "Record prompt, model name, settings, and generated responses",
    "Display comparison report showing both responses side by side"
  ],
  "file_structure": [
    "app.py - Main Streamlit application with UI and logic",
    "test_app.py - Unit tests for core functionality without network access",
  ...
  "plan": [
    {
      "id": "1",
      "goal": "Create app.py with Streamlit UI for task input and prompt variants",
      "inputs": [
        "Task description",
        "Prompt variant A",
        "Prompt variant B"
      ...
      "outputs": [
        "Streamlit form with input fields",
        "Submit button"
      ],
      "risk": "Low - standard Streamlit form creation"
    },
    {
      "id": "2",
      "goal": "Implement API call logic with fallback to deterministic mock",
      "inputs": [
        "Prompt text",
        "API key (optional)"
      ],
      ...
      "risk": "Medium - need to handle API errors gracefully and ensure mock is deterministic"
    },
    {
      "id": "3",
      "goal": "Build comparison report display",
      "inputs": [
        "Prompt A response",
        "Prompt B response",
      ...
      "risk": "Low - standard Streamlit display components"
    },
    {
      "id": "4",
      "goal": "Create test_app.py with unit tests",
      "inputs": [
        "Mock functions",
        "Test cases"
      ],
      ...
      "risk": "Medium - must ensure tests don't require network or API keys"
    },
    {
      "id": "5",
      "goal": "Write requirements.txt and README.md",
      "inputs": [
      ...
      "risk": "Low - standard documentation"
    }
  ]
}
```
...
Build a Streamlit app for comparing two LLM prompt variants with optional API integration and deterministic fallback, including a comparison report and unit tests.
  ...
  "plan": [
    {
      "id": "1",
      "goal": "Create app.py with Streamlit UI for task input and prompt variants",
      "inputs": ["Task description", "Prompt variant A", "Prompt variant B"],
      "outputs": ["Streamlit form with input fields", "Submit button"],
      "risk": "Low - standard Streamlit form creation"
    },
    {
      "id": "2",
      "goal": "Implement API call logic with fallback to deterministic mock",
      "inputs": ["Prompt text", "API key (optional)"],
    ...
    },
    {
      "id": "3",
      "goal": "Build comparison report display",
      "inputs": ["Prompt A response", "Prompt B response", "Metadata (model, settings)"],
      "outputs": ["Side-by-side comparison view", "Downloadable report"],
      "risk": "Low - standard Streamlit display components"
    },
    {
      "id": "4",
      "goal": "Create test_app.py with unit tests",
      "inputs": ["Mock functions", "Test cases"],
    ...
    },
    {
      "id": "5",
      "goal": "Write requirements.txt and README.md",
      "inputs": ["Project dependencies", "Setup instructions"],
      "outputs": ["requirements.txt file", "README.md documentation"],
      "risk": "Low - standard documentation"
    }
  ]
}
...
[
  {
    ...
    "purpose": "Main Streamlit application with UI, API call logic, deterministic mock fallback, and comparison report generation."
  },
  {
  ...
  },
  {
    ...
    "purpose": "Unit tests for core app functions without requiring network access or API keys."
  }
]
```
